...
(50-80uT from earth compared to 100-1000s of uT from the DC motors)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
class IMUDataReadings:
    ...
    def static_sensor_data(self, duration=10):
        ...
        # lists are initially used for lower time complexity with .append(),
        # but are converted to np.array for elemental math and filter input
        self.accel = np.array(self.accel_list) * self.accel_conv
        self.accel_means = np.mean(self.accel, axis=0)
        self.stdevs = np.std(self.accel, axis=0)
        self.gyro = np.array(self.gyro_list) * self.gyro_conv
        self.gyro_means = np.mean(self.gyro, axis=0)

        # Saves means for calibration of Accel and Gyro
        mean_data = np.array([self.accel_means, self.gyro_means])
    ...
    def take_reading(self, hz=562.5):
        ...
        try:
            ...
            print("\nEnding Data Read")
            self.raw_accel = np.array(self.accel_list)
            print(f"Raw Accel first 5 rows:\n {self.raw_accel[:5]}")
            self.accel = np.array(self.accel_list) * self.accel_conv
            self.gyro = np.array(self.gyro_list) * self.gyro_conv
            self.timestamp = np.array(self.timestamp_list)
            print(f"Scaled Accel first 5 rows:\n {self.accel[:5]}")
            print(f"Scaled Accel mean (all rows): {np.mean(self.accel, axis=0)}")
